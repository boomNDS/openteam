...
- Language: Python
- Approach: Split file processing into a reusable function, then use thread workers to handle each file. Each thread waits (with `sleep`) if the file doesn’t yet exist.
- Why: To support unit testing and follow prior guidance to use threading. Logic was split before the loop to improve reusability and testability.
- Time spent: ~2-3 hour
- **AI tools used:** [Zed AI](https://github.com/ZedAI) and [Cursor](https://www.cursor.so/) — both helped improve speed and autocompletion during development.
...
- Approach:
  ...
  - **Task 1:**  
    Use `GROUP BY` on `campa_id` to group records with the same ID. Then apply `SUM()` and `ROUND()` within the aggregation to compute totals cleanly.
  - **Task 2:**  
    Use a `CASE` expression to map raw country values into categories like `"Thailand"` and `"Global"` before performing a `GROUP BY` on the resulting country label.

- Why: By breaking the logic into smaller parts before combining it into a final SQL query, the code becomes easier to read, debug, and maintain.
- Time spent: ~50 min
- **AI tools used:** [ZedAI](https://github.com/ZedAI) for autocompletion and [ChatGPT](http://chat.openai.com) for query planning and explanation.
